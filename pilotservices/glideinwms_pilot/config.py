...
class Config(object):
    ...
    def __init__(self, config_ini="/etc/glideinwms/glidein-pilot.ini"):
        ...
        self.contextualization_type = self.ini.get(
            "DEFAULT", "contextualize_protocol")
        if self.contextualization_type is not None:
            self.contextualization_type = self.contextualization_type.upper()
        self.default_max_lifetime = self.ini.get(
            "DEFAULT", "default_max_lifetime", "172800")  # 48 hours
        self.max_lifetime = self.default_max_lifetime
        self.disable_shutdown = self.ini.getBoolean(
            "DEFAULT", "disable_shutdown", False)
        self.max_script_runtime = self.ini.get(
            "DEFAULT", "max_script_runtime", "60")

        self.pre_script_dir = self.ini.get(
            "DIRECTORIES", "pre_script_dir", "/usr/libexec/glideinwms_pilot/PRE")
        self.post_script_dir = self.ini.get(
            "DIRECTORIES", "post_script_dir", "/usr/libexec/glideinwms_pilot/POST")
    ...
    def setup_contextualization(self):
        raise NotImplementedError(
            'Implementation for contextualize_protocol %s not available' % self.contextualization_type)
...
class EC2Config(Config):
    ...
    def setup_contextualization(self):
        ...
        self.instance_metadata_url = self.ini.get(
            "DEFAULT", "instance_metadata_url")
        self.instance_userdata_url = self.ini.get(
            "DEFAULT", "instance_userdata_url")
...
class NimbusConfig(Config):
    ...
    def setup_contextualization(self):
        self.validate_contextualization()
        self.metadata_service_url_file = self.ini.get(
            "DEFAULT", "metadata_service_url_file")
